...
def data_db(cls):
    ...
    import logging
    logging.basicConfig(filename='/tmp/shakira.log', level=logging.DEBUG)
    if not (hasattr(cls, "datainstance") and cls.datainstance):
        ...
        if int(c.get("LOCAL", "model_type")) == defs.SILO_MODEL_AIO:
            ...
            logging.debug("checkkkkkkkkkkk")
            logging.debug("dbhost:%s"%(dbhost[0]))
    ...
    class tp_cdr(dict):
        ...
        def presentation_used(self):
            for obj_name in ["Tx Total Bytes", "Rx Total Bytes"]:
                try:
                    if not self[obj_name][str(STR_VIDEO)][str(SRC_AUX)]:
                         logger.debug("self[obj_name][STR_VIDEO][SRC_AUX] is null")
                    else:
                        if int(self[obj_name][str(STR_VIDEO)][str(SRC_AUX)]) > 0:
                            # try to return ASAP
                            return "Y"
            ...
            try:
                if not v["Duration"]["value"]:
                    ...
                    if(int(v["Duration"]["value"])>0):
                         logger.debug("Duration data is true with no space value and no 0 value")
                    else:
