...
logging.basicConfig(filename='C:/Users/sumitksi/Documents/LogTest/TPlog.log', level=logging.DEBUG)
app_name = "TP cdr Journal"
...
def data_db(cls):
    ...
    if not (hasattr(cls, "datainstance") and cls.datainstance):
        ...
        if int(c.get("LOCAL", "model_type")) == defs.SILO_MODEL_AIO:
            ...
            processhosts = dict([(v["Host"].split(":")[0], True) for v in localdbc.autofetchall_dict("""SHOW PROCESSLIST""")]).keys()
            if len(processhosts) < 1:
                logging.info("sumit testing 1")
                raise DatabaseError, "no connected em7 db host(s) listed"
            synchosts = localdbc.autofetch_column("""SELECT DISTINCT host FROM mysql.user WHERE user = 'db_sync_ssl'""")
            ...
            cls.datainstance = get_dbc(dbuser=c.get("LOCAL", "dbuser"), dbpass=c.get("LOCAL", "dbpasswd"), dbhost=dbhost[0], dbport=7706)

    return cls.datainstance

def reporting_db(cls, cred_details, my_logger=my_logger):
    from silo_common.credentials import dbc_from_cred_array
    return dbc_from_cred_array(cls.data_db(), my_logger)(cred_details)
...
def API_ALERT(message, did=did, dbc=dbc, my_logger=my_logger):
    if my_logger is not None:
        #my_logger.debug("CDR: (device id: %s): %s", did, message)
        logging.info("CDR: (device id: %s): %s", did, message)
    if dbc is not None:
...
try:
    ...
    pres_id = list(data_dbc.iterfetch("""SELECT
    journal_presentation_id, name
FROM master.dynamic_app_journal_presentation
WHERE app_id = %(app_id)s""", crunch_args))
    crunch_args.update(
            pres_map=", ".join("pres_%s AS %s" % p for p in pres_id),
            open_entries=", ".join(["%s"] * len(open_entries))
            )
...
WHERE entry_key IN (%(open_entries)s)
    ...
    logging.info("CDR_UPLOAD: crunched_calls_q(%s)", crunched_calls_q)
    crunched_calls = data_dbc.autofetch_all_assoc(
            crunched_calls_q,
            tuple(e.entry_key for e in open_entries),
            fields=("entry_key",)
            ) or {}
    if len(crunched_calls) > 0:
        logging.info("CDR_UPLOAD: got %s crunched calls" % (len(crunched_calls),))

    # add batch id and line number to each call,
    # put timestamps in datetime.datetime form
    for k, v in enumerate(crunched_calls):
        crunched_calls[v]["LINE_NUMBER"] = k + 1
        for dt_key in ["CALL_START_DTTM"]:
            datum = crunched_calls[v].get(dt_key, None)
            if not isinstance(datum, datetime.datetime):
                if isinstance(datum, basestring):
                    datum = datetime.datetime.strptime(
                            datum.translate(None, ":-/ "),
                            "%Y%m%d%H%M%S"
                            )
                else:
                    datum = datetime.datetime.utcfromtimestamp(0)
            #crunched_calls[v][dt_key] = datum.strftime("%Y-%m-%d %H:%M:%S")

    batch = dict(
            collector=dbc.autofetch_value("""SELECT
IFNULL(name, 'missing collector_id')
FROM master.system_settings_licenses""") or "missing collector_id",
            count=len(crunched_calls),
            name="%s_%s" % (
                    data_dbc.autofetch_value("""SELECT device
FROM master_dev.legend_device
WHERE id = %(did)s""", crunch_args) or "did(%(did)s)" % crunch_args,
                    int(time.time())
                    ),
            source=data_dbc.autofetch_value("""SELECT
IFNULL(url, 'missing stack_id')
FROM master.system_settings_core""") or "missing stack_id"
            )
    collection_times = [int(k.split("_")[0]) for k in crunched_calls]
    try:
        batch.update(
                begin=datetime.datetime.fromtimestamp(min(collection_times)),
                end=datetime.datetime.fromtimestamp(max(collection_times))
                )
    except ValueError:
        logging.info("CDR_UPLOAD: collection_times: %s" % (collection_times,))
        logging.info("CDR_UPLOAD: crunched calls: %s" % (crunched_calls,))

    # upload crunched calls
    try:
        rdbc = silo_cursor.reporting_db(cred_details)
        if not isinstance(rdbc, pl_cursor): raise Exception
    except:
        raise ReportingDBCredError("unable to get reporting database handle")
    rdb = rdbc.connection
    rdb.autocommit = False

    try:
        logging.info("CDR_UPLOAD: start transaction")
        rdb.begin()

        logging.info("CDR_UPLOAD: creating batch record")
        cdr_batch_q = """INSERT INTO %s
(BATCH_NAME, BATCH_BEGIN_DATE, BATCH_END_DATE,
    BATCH_SOURCE, BATCH_SOURCE_COLLECTOR, LOAD_DATE,
    ROW_COUNT, STATUS, STATUS_DATE)
VALUES (:name, :begin, :end,
    :source, :collector, SYSTIMESTAMP,
    :count, 'NEW', SYSTIMESTAMP)""" % (CDR_BATCH,)
        logging.info("CDR_UPLOAD: batch_query(%s)", cdr_batch_q)
        rdbc.execute(cdr_batch_q, batch)

        logging.info("CDR_UPLOAD: retrieving newly created batch id")
        batch.update(id=autofetch_value(rdbc,
                """SELECT %s.CURRVAL FROM DUAL""" % (CDR_BATCH_SEQ,)
                ))
    except:
    ...
    rdb_query_columns = dict(pres_id).values()
    ...
    try:
        logging.info("CDR_UPLOAD: inserting crunched calls")
        cdr_line_q = """INSERT INTO %(cdr_line)s (BATCH_ID, %(columns)s)
VALUES (%(id)s, :%(values)s)""" % batch
        logging.info("CDR_UPLOAD: executemany(%s)", cdr_line_q)
        rdbc.executemany(cdr_line_q, crunched_calls.values())

        logging.info("CDR_UPLOAD: committing transaction")
        rdb.commit()
    except:
        raise ReportingDBCommError(traceback.format_exc())

    batch.update(date=datetime.datetime.now())
    logging.info("CDR_UPLOAD: committed batch %(id)s at %(date)s", batch)
    ...
    for entry in open_entries:
        if entry.entry_key in crunched_calls:
            ...
            entry.update_collected_data({
                    "Reporting Batch ID": str(batch["id"]),
                    "Reporting Batch Name": batch["name"],
                    "Reporting Upload Time": batch["date"]
                    })
...
except NoCrunchedCalls:
    API_ALERT(e)
except (ReportingDBCredError,
        ReportingDBCommError):
    API_ALERT(e)
    try:
    ...
    logging.info("CDR_UPLOAD: %s", PROBLEM_STR)
    try:
...
#For CDR report the values are null while the values for the interface are N/A.    sumit bug for audio only call
    ...
    class tp_cdr(dict):
        ...
        _PROC_MAP = {
                ...
                "Rx Audio Protocol" : proc_duration_poly_val,
                "Rx Avg Latency" : proc_duration_poly_val,
        "Rx Video Protocol" : proc_duration_poly_val,
        "Rx Avg Jitter" : proc_duration_poly_val,
        "Rx Max Jitter" : proc_duration_poly_val,
        "Rx Max Latency" : proc_duration_poly_val,
        "Rx Avg Lost Pkts" : proc_duration_poly_val,
        "Rx Video Format" : proc_duration_poly_val,
        "Disconnect Reason 3rd party" : proc_duration_poly_val
        ...
        def to_entry(self):
            ...
            v = dict(self)
            v.setdefault("Call Termination Reason",v.get("State", proc_termination_val(0)))
            # JSON-ify complex types using dict::update vs. tp_cdr::update
            # for use with EM7_RESULT[].update_collected_data()
            import json
            v.update((i, json.dumps(j)) for i, j in v.iteritems()
                     if isinstance(j, dict) or isinstance(j, list))
            return v
        #need to change the code for presentation data y/n  sumit bug fixed for presentation_used


        def presentation_used(self):
            logging.info("testing for Presentation used")
            for obj_name in ["Tx Total Bytes", "Rx Total Bytes"]:
                try:
                    logging.info(int(self[obj_name]["1"]["4"]))
                    logging.info(int(self[obj_name]["1"]["4"]))
                    if not self[obj_name]["1"]["4"]:
                        logging.info("self[obj_name][1][4] is null")
                    else:
                        logging.info("not null")
                        if int(self[obj_name]["1"]["4"]) > 0:
                            # try to return ASAP
                            logging.info("Return Y")
                            return "Y"
                            #logging.info("end of try")
                except (KeyError, TypeError, ValueError):
                    #logging.info("except pass")
                    logging.info("following errors came")
                    logging.info(ValueError.__class__)
                    logging.info(ValueError.__doc__)
                    pass
            logging.info("Return N")
            return "N"

    class tp_device(object):

        _DEVICE_QUERY = """SELECT DISTINCT org.roa_id, org.company, dev.device, snmp.sysdescr,IFNULL(asset.asset_tag, '(no asset record)') AS asset_tag,IFNULL(loc.location, '(no asset record)') AS location FROM master_dev.legend_device AS dev LEFT JOIN master_dev.device_snmp_data AS snmp ON dev.id=snmp.did LEFT JOIN master.dynamic_app_collection AS coll ON dev.id=coll.did LEFT JOIN master_biz.organizations AS org on dev.roa_id=org.roa_id LEFT JOIN master_biz.legend_asset AS asset ON dev.id=asset.did LEFT JOIN master_biz.asset_location AS loc ON asset.id=loc.iid WHERE coll.app_id = %s AND dev.id = %s"""
        def __init__(self, did, app_id, dbc, my_logger):
            ...
            if device_row is None:
                raise DeviceNotAligned("no device with did: %s found aligned with this app" %(self.did,))
            self.__dict__.update(device_row)
            self.model = ""

        def _get_app_id(self, app_guid):
            return self.dbc.autofetch_value("""SELECT aid
...
WHERE app_guid = %s""", (app_guid,))
        ...
        def itercalls(self):
            ...
            #self.logging.info("CDR_JOURNAL: got %d calls", len(self.call_data))
...
FROM (
    ...
    LIMIT 1
        ...
        def _load_call_indices(self):
            from silo_common.exceptions import ProgrammingError
            try:
                self.call_index = self.dbc.autofetch_all_assoc(
                        self._INDEX_QUERY % (self.date_range,),
                        fields=("collection_time", "grp", "ind")
                        )
        ...
        def _load_call_data(self):
            ...
            try:
                ...
                self.local_number = self.dbc.autofetch_value(
                        self._LOCAL_NUMBER_QUERY % dict(
...
WHERE app_id = %%s AND name in %s""" % (tuple(self._OBJ_MAP.keys()),),
            ...
            #self.logging.info(self._DATA_QUERY)
...
WHERE did = %s AND state = %s AND date_update %s""" % (
            ...
            for row in self.dbc.iterfetch_d(
                ...
                if entry_key in self.call_data:
                    self.logging.info("CDR_JOURNAL: entry %s already found, updating" % (entry_key,))
                    #ogging.info("sumit 12 CDR_JOURNAL: entry %s already found, updating" % (entry_key,))
                else:
                    #self.logging.info("CDR_JOURNAL: adding entry %s" % (
                    #        entry_key,
                    #        ))
                ...
                self.logging.info("row[type]= %s", row["Type"])
                if row["Type"] == "Video":
...
WHERE did = %s AND state = %s AND date_update %s""" % (
    ...
    try:
        ...
        logging.info("CDR_JOURNAL: %s", app_meta)

        app = dict((t._APP_GUID, t) for t in (
                cisco_tp_device, tandberg_c, tandberg_mxp, polycom
                ))
        collect = data_dbc.autofetch_value("""SELECT app_guid
...
GROUP BY app_guid""" % (did, tuple(app.keys())))
        ...
        tpd = app[collect](did, app_id, data_dbc, my_logger)
        ...
        logging.info("sumit testing Zero calls")
        for k, v in tpd.itercalls():
            try:
                logging.info("sumit 1.1 duration v[duration][value] in string format = %s", v["Duration"]["value"])
                logging.info("sumit 1.2 duration int(v[duration][value]) in integer format = %d", int(v["Duration"]["value"]))
                logging.info("sumit 1.3 duration value length len(v[duration][value])  = %d", len(v["Duration"]["value"]))
                if not v["Duration"]["value"]:
                    logging.info("Duration has NULL value")
                    continue
                else:
                    if(int(v["Duration"]["value"])>0):
                        logging.info("Duration data is true with no space value and no 0 value")
                    else:
                        logging.info("Duration has zero value")
                        continue
            except ValueError:
                # or calls with no duration
                logging.info("Following are the ValueError")
                logging.info(ValueError.__class__)
                logging.info(ValueError.__doc__)
                logging.info("so cintinue with new line")
                continue
            entry = get_entry(k)
            if entry is None:
                #logging.info("CDR_JOURNAL: no entry found for key %s", k)
            ...
            logging.info("v.to_entry = ", v.to_entry())
            calls.append(entry)
            logging.info("sumit 7 entry done")

    except MySQLError as e:
        raise TelePresenceCommError(str(e))

    finally:
        logging.info("sumit 8 finally")
        logging.info("CDR_JOURNAL: %s", app_meta)
        set_app_instance_meta_data(app_meta)
        if len(calls) > 0:
            logging.info("CDR_JOURNAL: saving %d calls", len(calls))
            EM7_RESULT.extend(calls)
            logging.info("CDR_JOURNAL: EM7_RESULT contains %s calls", len(EM7_RESULT))
...
except (CallAlreadyClosed,
        DeviceNotAligned,
        TelePresenceCommError,
        TelePresenceCredError) as e:
    API_ALERT(e)
except:
    COLLECTION_PROBLEM = True
    import traceback
