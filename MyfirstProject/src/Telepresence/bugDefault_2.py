...
my_logger = dynapp_log.minimal_logger()

from silo_common.logger import minimal_logger
my_logger = minimal_logger()

logging.basicConfig(filename='/tmp/TP_CDR_JOURNAL.log', level=logging.DEBUG)
...
def data_db(cls):
    ...
    if not (hasattr(cls, "datainstance") and cls.datainstance):
        ...
        if int(c.get("LOCAL", "model_type")) == defs.SILO_MODEL_AIO:
            ...
            cls.datainstance = get_dbc(
                    dbuser=c.get("LOCAL", "dbuser"),
                    dbpass=c.get("LOCAL", "dbpasswd"),
                    dbhost=dbhost[0],
                    dbport=7706
                    )
...
WHERE entry_key IN (%(open_entries)s)
    ...
    my_logger.debug("CDR_UPLOAD: crunched_calls_q(%s)", crunched_calls_q)
    ...
    if len(crunched_calls) > 0:
        my_logger.debug("CDR_UPLOAD: got %s crunched calls" % (len(crunched_calls),))
...
FROM master.system_settings_core""") or "missing stack_id"
    ...
    try:
        ...
        my_logger.debug("CDR_UPLOAD: collection_times: %s" % (collection_times,))
        my_logger.debug("CDR_UPLOAD: crunched calls: %s" % (crunched_calls,))
    ...
    try:
        my_logger.debug("CDR_UPLOAD: start transaction")
        rdb.begin()

        my_logger.debug("CDR_UPLOAD: creating batch record")
...
VALUES (:name, :begin, :end,
    ...
    :count, 'NEW', SYSTIMESTAMP)""" % (CDR_BATCH,)
        my_logger.debug("CDR_UPLOAD: batch_query(%s)", cdr_batch_q)
        rdbc.execute(cdr_batch_q, batch)

        my_logger.debug("CDR_UPLOAD: retrieving newly created batch id")
    ...
    try:
        my_logger.debug("CDR_UPLOAD: inserting crunched calls")
        cdr_line_q = """INSERT INTO %(cdr_line)s (BATCH_ID, %(columns)s)
VALUES (%(id)s, :%(values)s)""" % batch
        my_logger.debug("CDR_UPLOAD: executemany(%s)", cdr_line_q)
        rdbc.executemany(cdr_line_q, crunched_calls.values())

        my_logger.debug("CDR_UPLOAD: committing transaction")
        rdb.commit()
    except:
        raise ReportingDBCommError(traceback.format_exc())

    batch.update(date=datetime.datetime.now())
    my_logger.debug("CDR_UPLOAD: committed batch %(id)s at %(date)s", batch)
    ...
    my_logger.debug("CDR_UPLOAD: %s", PROBLEM_STR)
    try:
    ...
    class tp_cdr(dict):
        ...
        def presentation_used(self):
            ...
            for obj_name in ["Tx Total Bytes", "Rx Total Bytes"]:
                #logging.info("object name read by for loop")

                try:
                    #logging.info("entered here in try ")

                    #logging.info("obj name = %s", obj_name)
                    #logging.info(type(obj_name).__name__)
                    #logging.info(type(self[obj_name]).__name__)

                    #logging.info("STR_VIDEO = %d", STR_VIDEO)
                    #logging.info(type(STR_VIDEO).__name__)

                    #logging.info("SRC_AUX = %d", SRC_AUX)
                    #logging.info(type(SRC_AUX).__name__)

                    #logging.info("test 1")
                    #logging.info(self[obj_name]["1"])
                    #logging.info(self[obj_name]["1"]["4"])
                    #logging.info(type(self[obj_name]["1"]["4"]).__name__)
                    #logging.info(self[obj_name][(STR_VIDEO).__str__()][(SRC_AUX]).__str__())
                    #logging.info(self[obj_name]['STR_VIDEO']['SRC_AUX'])
                    #logging.info(type(self[obj_name][STR_VIDEO]).__name__)
                    #logging.info(self[obj_name][1][4])
                    #logging.info("test 2")
                    #logging.info(type(self[obj_name][STR_VIDEO][SRC_AUX]).__name__)
                    #logging.info("%s",(self[obj_name][STR_VIDEO][SRC_AUX]).__str__())
                    #if int(self[obj_name][STR_VIDEO][SRC_AUX]) > 0:
                    #logging.info(self[obj_name][str(STR_VIDEO)][str(SRC_AUX)])
                    #logging.info("test 3 int value for final result")
                    #logging.info(int(self[obj_name][str(STR_VIDEO)][str(SRC_AUX)]))
                    if self[obj_name]["1"]["4"] != "0":
                        # try to return ASAP
...
WHERE app_guid = %s""", (app_guid,))
        ...
        def itercalls(self):
            ...
            #self.my_logger.debug("CDR_JOURNAL: got %d calls", len(self.call_data))
...
WHERE app_id = %%s AND name in %s""" % (tuple(self._OBJ_MAP.keys()),),
            ...
            #self.my_logger.debug(self._DATA_QUERY)
...
WHERE did = %s AND state = %s AND date_update %s""" % (
            ...
            for row in self.dbc.iterfetch_d(
                ...
                if entry_key in self.call_data:
                    self.my_logger.debug("CDR_JOURNAL: entry %s already found, updating" % (entry_key,))
                    #ogging.info("sumit 12 CDR_JOURNAL: entry %s already found, updating" % (entry_key,))
                else:
                    #self.my_logger.debug("CDR_JOURNAL: adding entry %s" % (
                    #        entry_key,
                    #        ))
                ...
                self.my_logger.debug("row[type]= %s", row["Type"])
                if row["Type"] == "Video":
...
WHERE did = %s AND state = %s AND date_update %s""" % (
    ...
    try:
        ...
        my_logger.debug("CDR_JOURNAL: %s", app_meta)
...
GROUP BY app_guid""" % (did, tuple(app.keys())))
        ...
        for k, v in tpd.itercalls():
            ...
            entry = get_entry(k)
            if entry is None:
                #my_logger.debug("CDR_JOURNAL: no entry found for key %s", k)
