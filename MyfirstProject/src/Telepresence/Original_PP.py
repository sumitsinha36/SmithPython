...
def reporting_db(cls, cred_details, logger=logger):
    ...
    return dbc_from_cred_array(cls.data_db(), logger)(cred_details)
...
VALUES (:name, :begin, :end,
    ...
    try:
        logger.debug("CDR_UPLOAD: inserting crunched calls")
...
VALUES (%(id)s, :%(values)s)""" % batch
    ...
    logger.debug("CDR_UPLOAD: committed batch %(id)s at %(date)s", batch)
...
#For CDR report the values are null while the values for the interface are N/A.    sumit bug for audio only call
    ...
    class tp_cdr(dict):
        ...
        #code for presentation data Y/N  fixed for presentation_used
        def presentation_used(self):
            for obj_name in ["Tx Total Bytes", "Rx Total Bytes"]:
                try:
                    logger.debug(self[obj_name][str(STR_VIDEO)][str(SRC_AUX)])
                    if self[obj_name][str(STR_VIDEO)][str(SRC_AUX)] != "0":
                        # try to return ASAP
                        return "Y"
...
WHERE did = %s AND state = %s AND date_update %s""" % (
            ...
            for row in self.dbc.iterfetch_d(
                ...
                self.logger.debug("row[type]= %s", row["Type"])
                if row["Type"] == "Video":
...
GROUP BY app_guid""" % (did, tuple(app.keys())))
        ...
        tpd = app[collect](did, app_id, data_dbc, logger)
        ...
        for k, v in tpd.itercalls():
            try:
                if v["Duration"]["value"] != '':
                    logger.debug("Not a Zreo Length Call")
                else:
                    logger.debug("Zero Length call")
                    continue
            except ValueError:
