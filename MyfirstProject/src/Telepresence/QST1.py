...
GROUP BY app_guid""" % (did, tuple(app.keys())))
        ...
        for k, v in tpd.itercalls():
            ...
            my_logger.debug("v.to_entry = ", v.to_entry())
            calls.append(entry)
            my_logger.debug("sumit 7 entry done")
        ...
        set_app_instance_meta_data(app_meta)
        if len(calls) > 0:
            my_logger.debug("CDR_JOURNAL: saving %d calls", len(calls))
            EM7_RESULT.extend(calls)
            my_logger.debug("CDR_JOURNAL: EM7_RESULT contains %s calls", len(EM7_RESULT))
